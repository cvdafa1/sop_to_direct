...
`compile_program` 以返回 `code`/`msg` 判定。成功则进 Step 6。失败对照 `references/direct_compile_error_codes.md` 分类。**错误信息里只要包含格式类，就只修这些格式类错误**（可与非格式类同时出现）；**完全不含格式类**则不重试，直接 Step 6。

**重试硬约束（全程）：**

- **只更新原程序**：全程使用首次 `create_program` 得到的同一主 appid（及已创建的子程序）；只允许 `save_program` / `compile_program`；**禁止**再调 `create_program`、禁止新建主/子程序、禁止换 appid「另开一份」
- **禁止为通过编译而简化程序**：不得删节点、合并多步、去掉分支/否则、改串行少连线、减少 `flow:subproc`、用概括替代多操作（同 Step 3 失败处置）
- 禁止顺手改位号或其他非格式项；禁止改拓扑（节点/边集合相对 SOP 语义不得收缩）

**格式类**（可自修，同 appid 最多 3 次）：问题在 IR/XML 写法，不在位号对不对、也不在平台缺资源。

| 组 | 内容 | 代表错误码 |
|----|------|------------|
| 必填空缺 | `ext` 缺字段或空串；值须来自 SOP/已有 IR，禁止编造 | `-243009112` `-204447099` `-35675313` `-31848101` `-208993724` `-247818342` `-3030983` `-170079177` |
| 名字写法 | `#()` / `$()` / `$$()` 包裹、首字符、字符集、常量位数、表达式长度 | `-219754247` `-219447726` `-192235230` `-212692174` `-260966457` `-96338341` `-188494599` `-41750206` |
| 声明与类型 | timers/variables/参数未声明、计时器当变量、数据类型或比较符 | `-216790910` `-126741904` `-34899616` `-180052257` `-5232440` `-172107283` `-196493616` `-211321189` |
| 骨架与连线 | 起止步、并行结束条件、分支未连接/交叉、输入输出路数 | `-213854666` `-18762285` `-167238399` `-161829759` `-149689194` `-190130525` `-74702161` |
| 子程序引用 | 子程序空、重名、不存在、引用与当前子程序不一致 | `-210016328` `-190444754` `-88217109` `-191793060` `-97677076` |

自修顺序：只改格式类对应字段 → `ir_to_xml` + `validate_bpmn` 退出码 0 → **`save_program`（同一 appid，更新原程序）** → 再 `compile_program`。对照 `ir_schema.md`、`element_schema.json`、`golden_xml_rules.md`、`subprocess.md`、`node_reference.md` §一。同一轮报错里的非格式项保持不动。**超过 3 次仍含格式类或仍失败：停止自修，必须进 Step 6 出报告**（写明已重试 3 次、剩余 `code`/`msg`）；禁止继续第 4 次改/save/compile；**仍禁止新建程序或简化流程**。

**不含格式类时**（仅位号不存在/无效/类型不符/空位号、设备与枚举、文件与扫码、算法与请求、别名与物料，或无法归入上表）：不改 IR、不 `save`、不再编译，**必须进 Step 6 出报告**。报告须含 `code`、`msg`、错误码常量（若能对应）和涉及对象。

### Step 6 — 报告

程序名、appid、节点/连线数、编译结果、补充项。  
**编译失败（含：无格式类未重试 / 格式类已满 3 次仍失败）一律出本报告**，写明：是否含格式类、格式类已重试次数（0 或 ≤3）、剩余 `code`/`msg`。
...
| 平台编译错误码 | `references/direct_compile_error_codes.md` |
