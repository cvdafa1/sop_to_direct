...
| **设备/管路** | 涉及的设备名称和位号 | "反应釜R-01"、"循环水泵P0801A"、"出口开关阀EV_0801A2" |
...
| 字段 | 说明 | 命名规则 | 示例 |
|------|------|----------|------|
| 程序名称(program_name) | 主程序名称 | 仅支持字母、数字、下划线，必须字母开头，不允许中文 | sop_shutdown_circ_water |
...
**流程**：展示建议值 → 用户确认或修改 → 用户明确同意后才调用 API。用户未同意前禁止调用 create_program。
...
|------------|----------|-----------|----------|
| flow:start, flow:end, io:*, timer:wait, timer:start, timer:restart, timer:stop, msg:* | **plain** | 无 | `{"lineType":1}` |
| flow:and, flow:or, timer:cond 的"是"分支 | **situation_yes** | `name="条件成立"` | `{"situation":"yes"}` |
| flow:and, flow:or, timer:cond 的"否"分支 | **situation_no** | `name="条件不成立"` | `{"situation":"no"}` |
| flow:branch 的每个分支 | **branch_option** | `name="{选项名}"` | `{"situation":"{索引}"}` |
...
情况1：源和目标在同一列（x 中心差 < 10px）
  → 垂直直连
  waypoint1 = (src.cx, src.bottom)     # 源底部中心
  waypoint2 = (tgt.cx, tgt.top)        # 目标顶部中心

情况2：源和目标不在同一列
  → L 形路径（3~4 个 waypoint）
  mid_y = (src.bottom + tgt.top) / 2   # 中间 y 坐标
  waypoint1 = (src.cx, src.bottom)     # 源底部中心
  waypoint2 = (src.cx, mid_y)          # 垂直下走
  waypoint3 = (tgt.cx, mid_y)          # 水平移动
  waypoint4 = (tgt.cx, tgt.top)        # 垂直进入目标

情况3：从并行容器到外部节点
  → 穿越容器边界的路径
  从容器 outgoing 的 parallelEnd 底部出发
  waypoint1 = (pend.cx, pend.bottom)
  waypoint2 = (pend.cx, pend.bottom + 50)  # 离开容器
  waypoint3 = (tgt.cx, pend.bottom + 50)   # 水平移动
  waypoint4 = (tgt.cx, tgt.top)            # 进入目标
```

**连线避让规则**：
- mid_y 必须落在两个节点之间的空白区域（不与任何节点 y 范围重叠）
- 若 mid_y 落在某节点内，调整为该节点下方 + 20px
...
gen.layout_parallel(
    branch_groups=[
        ["dcs_pump_a", "or_pump_a", "dcs_valve_a", "or_valve_a"],
        ["dcs_pump_b", "or_pump_b", "dcs_valve_b", "or_valve_b"],
        ["dcs_pump_c", "or_pump_c", "dcs_valve_c", "or_valve_c"],
    ],
    start_x=100, start_y=280
)
# 自动设置容器边界
gen.layout_container("parallel_1", [
    "dcs_pump_a", "or_pump_a", "dcs_valve_a", "or_valve_a",
    "dcs_pump_b", "or_pump_b", "dcs_valve_b", "or_valve_b",
    "dcs_pump_c", "or_pump_c", "dcs_valve_c", "or_valve_c",
])
...
- `layout_parallel()`：多分支水平排列，300px 间距
- `layout_container()`：计算容器边界
- `calc_waypoints()`：L 形路径 + 避让节点
- `calc_u_waypoints()`：U 形绕行（复杂场景）
- `check_overlaps()`：验证无重叠

#### 3.6 完整元素 schema 参考

节点类型的完整 schema 定义见 `references/element_schema.json`，包含：
- 18 种节点类型的 XML 模板、ext:data schema、必选/可选字段
- sequenceFlow 4 种变体（plain/situation_yes/situation_no/branch_option）
- 连线选择规则（按源节点类型确定变体）
...
- **严格限制：禁止创建任何新程序（包括测试程序）** — 只允许在本次已创建的程序（同一 appid）上修正 XML，重新 save_program → compile_program
- create_program 在整个流程中只允许调用一次
...
**最大重试 3 次**，每次重试前：
  ...
  4. 修正后重新 save_program → compile_program
...
3. **并行处理**：当 SOP 中有"同时"操作多台设备时，使用 `flow:parallel1` 容器
