...
| 是/否两条路 | **只有「是/否」两条路**时：任一成立用 `flow:or`；须同时成立用 `flow:and`。无否则时只连 yes，不硬凑否 | `flow:or` / `flow:and` |
| 择一分支 | 多于「是/否」两条路时（N≥2，不限 3）。**每个分支各自**设 `relation`：`"or"` 或 `"and"`，同一节点里可以混用，不必全部同一种 | `flow:branch` |
...
| R12 | "分别判断多种情况/哪台运行" | `flow:branch`（N 支；每支 `relation` 自定；**`branch[].row`=0..N-1**） | 都用择一；`row` 与下标、出边 `situation` 一致 |
