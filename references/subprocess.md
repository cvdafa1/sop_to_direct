...
| 对象 | 名称 | 描述 |
|------|------|------|
| **主程序** | 对话已给出 → 用用户名（规范为 `[A-Za-z0-9_]`）；未给出 → 按文档标题/内容生成 | **始终**按文档内容生成（Step 2 展示时可改） |
| **子程序** | Agent 按 Step 1.5 拆分边界自行拟定 name | Agent 按该子职责/SOP 阶段自行拟定描述；写入 `subTitle` |
...
**Step 1.5**：必须对话编号二选一，且标明唯一推荐项（不得只给选项不标推荐；禁止宿主单选控件）。文案见 `interaction.md`。本文件只规定评估、拆分原则、save 与 `flow:subproc`。
...
草案由 Agent 按拆分自行生成；用户确认（或改过）后即定稿。

| Step 1.5 字段 | 写入位置 | 规则 |
|---------------|----------|------|
| `name` | XML 属性 `name`，且与 save 子程序 `name` 一致 | 仅 `[A-Za-z0-9_]`（与主程序/timers/variables 同规则）；按拆分职责生成 |
| `描述/职责` | `ext:data.subTitle`（必填，禁止 `""`） | 按拆分职责/文档内容生成；与 1.5 确认文案一致 |
